        ...
        const fetchingData = async () => {
            ...
            try {
                const [data, pageData] = await Promise.all([
                    getFetchData(`${SERVER_URL}/images/sub-categories/${slug}/?page=${currentPage}`, {
                        next: { revalidate: 180 },
                    }),
                    getFetchData(`${SERVER_URL}/images/sub-categories/${slug}/page_data`, {
                        next: { revalidate: 180 },
                    }).catch(() => null), // don't fail if page_data is missing
                ]);
                ...
                <div className="flex flex-col flex-wrap gap-y-5 lg:gap-y-10 w-full">
                    ...
                    <div className="flex flex-col flex-wrap w-full gap-y-3">
                        <h1 className="text-2xl font-bold text-center">
                            {pageContent?.intro_heading || `All PNG Image ${subCategoryData?.name}s - Free Transparent Downloads`}
                        </h1>
                    ...
                    <div className="flex flex-col flex-wrap w-full">
                                ...
                                {images && images.map((image: any) => {
                                    ...
                                    if (image.keywords) {
                                        if (Array.isArray(image.keywords)) {
                                            processedKeywords = image.keywords.map((item: any) => item.name || item).filter(Boolean);
                                        } else if (typeof image.keywords === 'string') {
                                            processedKeywords = image.keywords.split(",").map((k: string) => k.trim()).filter(Boolean);
                                        }
                                    }
                                    ...
                                    const imageSchema = {
                                        ...
                                        "@graph": [
                                            ...
                                            },
                                            {
                                                ...
                                                "creator": { "@type": "Organization", "name": siteConfig.siteName, "url": siteConfig.url },
                                                ...
                                                "copyrightHolder": { "@type": "Organization", "name": siteConfig.siteName },
                                            },
                                        ],
                                    };
                        ...
                        {/* ── PAGINATION TEXT ── */}
                        {paginationText && (
                            <p className="text-center text-gray-500 text-sm mt-4">{paginationText}</p>
                        )}
                    ...
                    {pageContent?.seo_heading && (
                        <div className="flex flex-col gap-y-4 w-full mt-4">
                            ...
                            {pageContent.seo_paragraph_1 && <p className="text-gray-600 text-sm lg:text-base leading-relaxed">{pageContent.seo_paragraph_1}</p>}
                            {pageContent.seo_paragraph_2 && <p className="text-gray-600 text-sm lg:text-base leading-relaxed">{pageContent.seo_paragraph_2}</p>}
                            {pageContent.seo_paragraph_3 && <p className="text-gray-600 text-sm lg:text-base leading-relaxed">{pageContent.seo_paragraph_3}</p>}

                            {pageContent.popular_uses_heading && pageContent.popular_uses && pageContent.popular_uses.length > 0 && (
                                <div className="mt-2">
                                    <h3 className="text-lg font-semibold mb-2">{pageContent.popular_uses_heading}</h3>
                                    <ul className="list-disc list-inside space-y-1">
                                        {pageContent.popular_uses.map((use, index) => (
                                            <li key={index} className="text-gray-600 text-sm lg:text-base">{use}</li>
                                        ))}
                                    </ul>
                                </div>
                            )}
                    ...
                    {pageContent?.faqs && pageContent.faqs.length > 0 && (
                        <div className="flex flex-col gap-y-4 w-full mt-4">
                            <h2 className="text-xl font-bold">{pageContent.faq_heading || "Frequently Asked Questions (FAQ)"}</h2>
                            <div className="flex flex-col gap-y-4">
                                {pageContent.faqs.map((faq, index) => (
                                    <div key={index} className="border border-gray-200 rounded-xl p-4 shadow-sm">
                                        <h4 className="font-semibold text-gray-800 text-sm lg:text-base mb-1">{faq.question}</h4>
